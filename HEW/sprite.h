...
public:
	...
	//	描画 (引数：テクスチャ、カラー設定(R)、カラー設定(G)、カラー設定(B)、カラー設定(A)、X座標、Y座標、テクスチャX座標、テクスチャY座標、テクスチャの横幅、テクスチャの縦幅)
	void Draw(TextureIndex texture_index, int col_r, int col_g, int col_b, int col_a, float dx, float dy, int tx, int ty, int tw, int th);
